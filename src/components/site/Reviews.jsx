...
export const Reviews = () => {
  return (
    <section data-testid="reviews" id="recensioni" className="bg-cream-alt py-24 lg:py-36 bg-grain">
        ...
        <Reveal className="text-center max-w-3xl mx-auto mb-16">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            <div data-testid="rating-google" className="flex items-center gap-3">
              ...
              <span className="text-sm text-text-secondary"><strong className="text-dark">{INFO.googleRating.value}/5</strong> · Google ({INFO.googleRating.count} recensioni)</span>
            </div>
            ...
            <div data-testid="rating-tripadvisor" className="flex items-center gap-3">
              ...
              <span className="text-sm text-text-secondary">TripAdvisor — {INFO.tripadvisorBadge}</span>
            </div>
          </div>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-olive/15 border border-olive/15">
          {REVIEWS.map((r, i) => (
            <Reveal key={i} delay={(i % 3) * 0.1} data-testid={`review-card-${i}`} className="bg-cream p-9 flex flex-col">
              <Quote className="text-olive mb-5" size={30} strokeWidth={1.2} />
              <p className="font-heading text-xl lg:text-2xl text-dark leading-snug italic flex-1">
                “{r.quote}”
              </p>
              <div className="mt-7 pt-5 border-t border-olive/15">
